...
import { headingFixSnippet } from "../suggestion-snippets";
  ...
  check(context: RuleContext): A11yFinding[] {
    ...
    walkJsxElements(context.ast, (path: NodePath<t.JSXElement>) => {
      ...
      if (lastLevel > 0 && level > lastLevel + 1) {
        ...
        const suggestedTag = `h${lastLevel + 1}`;
        const headingText = getTextContent(path.node);
        findings.push({
          ...
          column: loc.column,
          element: tag,
          suggestion: `Use ${suggestedTag} instead of ${tag} to maintain logical heading order.`,
          fixSnippet: headingFixSnippet(suggestedTag, headingText),
        });
      }
      lastLevel = level;
    });
